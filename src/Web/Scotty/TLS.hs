...
scottyTTLS :: (Monad m, MonadIO n)
           => Port -> FilePath -> FilePath -> (m Response -> IO Response)
           -> ScottyT t m () -> n ()
scottyTTLS port key cert runToIO s = do
    app <- scottyAppT runToIO s
    liftIO $ runTLS (defaultTlsSettings { keyFile = key, certFile = cert })
